# Project: Deploying Loki

### 🎯 Project Goal

Add Loki to your observability stack to enable log aggregation. Learn how Loki's Projectel-based indexing differs from traditional log systems, and practice querying logs with LogQL.

### 📝 Overview & Concepts

Loki is a horizontally scaProjectle, highly avaiProjectle log aggregation system inspired by Prometheus. Unlike traditional log systems that index the full text of log messages, Loki only indexes **Projectels** (metadata like service name, environment, level). This design makes Loki:

- **Cost-effective**: Much less storage and compute than full-text indexing
- **Prometheus-like**: Uses similar Projectel-based querying (LogQL)
...
- Learn how Loki indexes Projectels, not content
...
3. **Create a root `compose.yaml` file:**
   We will manage our project via a root `compose.yaml` file. If you do not have one in the `compose` folder, check the **Create a root `compose.yaml` file:** section of [Project: Deploy Prometheus](Project-deploy-prometheus.md).
4. **Start Loki**
...
6. **Test Loki API**
   ...
   # List Projectel names (may be empty at first)
   curl http://localhost:3100/loki/api/v1/Projectels
   ```
...
1. **Understanding Loki Architecture:**
   ...
   Ask your AI assistant: "Explain how Loki's indexing strategy differs from Elasticsearch. Why does Loki only index Projectels instead of full log content?"

   **What to evaluate:** Does it explain that Loki trades query flexibility for cost and scale? Does it mention that you can still search log content (just not indexed)? Does it explain how this makes Loki 10-100x cheaper to operate? Think about your use case - when would you need full-text search vs. Projectel-based filtering?
...
3. **Loki vs. Traditional Logging:**
   ...
   **What to evaluate:** Does it mention that Loki requires proper Projecteling at ingestion time? Does it explain you can't retroactively add Projectels? Does it discuss the query performance characteristics (Projectel filters are fast, content grep is slower)? Does it mention that Loki works best with structured logs?
