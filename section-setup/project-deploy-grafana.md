...
6. **Explore Prometheus Metrics**
   ...
   - Try these queries in the query builder:
     ...
     - `up{job="loki"}` - Filter by job Projectel
...
2. **Understanding Correlation:**
   ...
   **What to evaluate:** Does it explain that Grafana extracts these tag values from trace spans and uses them to construct LogQL queries? Does it mention that when viewing a trace, you'll see a link to related logs? Does it explain that this requires consistent Projecteling: your traces and logs must both have matching 'job' and 'instance' Projectels?
...
3. **Data Source Settings:**
   ...
   **What to evaluate:** Does it explain that 'timeInterval' sets the minimum step size for range queries, aligning with Prometheus scrape intervals? Does it mention that 'httpMethod: POST' is useful for queries with long URLs or many Projectels? Does it explain why matching timeInterval to your scrape interval (15s in this Project) prevents querying for data that doesn't exist?
